...
; 任务
LABEL_DESC_LDT0:	Descriptor	       0,       LDTLen0 - 1, DA_LDT						; LDT0
LABEL_DESC_TSS0:	Descriptor	       0,       TSSLen0 - 1, DA_386TSS					; TSS0
...
SelectorLDT0		equ	LABEL_DESC_LDT0		- LABEL_GDT			; LDT0 选择子
SelectorTSS0		equ	LABEL_DESC_TSS0		- LABEL_GDT			; TSS0 选择子
SelectorLDT1		equ	LABEL_DESC_LDT1		- LABEL_GDT			; LDT1 选择子
SelectorTSS1		equ	LABEL_DESC_TSS1		- LABEL_GDT			; TSS0 选择子
; END of [SECTION .gdt]
...
; 实模式下使用这些符号
; 字符串
_szPMMessage:			db	"In Protect Mode now. zzzzrrrrtttt@R", 0Ah, 0Ah, 0			; 进入保护模式后显示此字符串
...
[BITS	32]
LABEL_T1_DATA:
_szTask1Message:	db	"MRSU", 0Ah, 0Ah, 0
...
LABEL_MEM_CHK_OK:
	...
	mov	word [LABEL_DESC_TASK0_DATA + 2], ax
	shr	eax, 16
	mov	byte [LABEL_DESC_TASK0_DATA + 4], al
	mov	byte [LABEL_DESC_TASK0_DATA + 7], ah
	
	xor	eax, eax
	mov	ax, cs
	shl	eax, 4
	add	eax, LABEL_T0_CODE
	mov	word [LABEL_DESC_TASK0_CODE + 2], ax
	shr	eax, 16
	mov	byte [LABEL_DESC_TASK0_CODE + 4], al
	mov	byte [LABEL_DESC_TASK0_CODE + 7], ah
	
	xor	eax, eax
	mov	ax, cs
	shl	eax, 4
	add	eax, LABEL_T0_STACK0
	...
	mov	ax, cs
	shl	eax, 4
	add	eax, LABEL_T0_STACK3
	mov	word [LABEL_DESC_TASK0_STACK3 + 2], ax
	shr	eax, 16
	mov	byte [LABEL_DESC_TASK0_STACK3 + 4], al
	mov	byte [LABEL_DESC_TASK0_STACK3 + 7], ah
	...
	mov	word [LABEL_DESC_TASK1_DATA + 2], ax
	shr	eax, 16
	mov	byte [LABEL_DESC_TASK1_DATA + 4], al
	mov	byte [LABEL_DESC_TASK1_DATA + 7], ah
	
	xor	eax, eax
	mov	ax, cs
	shl	eax, 4
	add	eax, LABEL_T1_CODE
	mov	word [LABEL_DESC_TASK1_CODE + 2], ax
	shr	eax, 16
	mov	byte [LABEL_DESC_TASK1_CODE + 4], al
	mov	byte [LABEL_DESC_TASK1_CODE + 7], ah
	
	xor	eax, eax
	mov	ax, cs
	shl	eax, 4
	add	eax, LABEL_T1_STACK0
	...
	mov	ax, cs
	shl	eax, 4
	add	eax, LABEL_T1_STACK3
	mov	word [LABEL_DESC_TASK1_STACK3 + 2], ax
	shr	eax, 16
	mov	byte [LABEL_DESC_TASK1_STACK3 + 4], al
	mov	byte [LABEL_DESC_TASK1_STACK3 + 7], ah
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	mov		al, [_SavedIMREG]			; 恢复中断屏蔽寄存器的值
	out		21h, al						
...
.1:
	...
	push	SelectorTask0Stack0	
	push	TopOfTask0Stack0	
	pushfd						
	pop		eax					
	or		eax, 0x200			
	push	eax					
	push	SelectorTask0Code	
	push	0					
	iretd
...
ClockHandler	equ	_ClockHandler - $$
	...
	cmp		dword [dwCurrentTask], 0	; 根据 dwCurrentTask 完成任务切换
	je		.1							
